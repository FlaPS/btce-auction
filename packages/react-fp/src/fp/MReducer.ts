...
type ReducerBoxType<S, A> = {
  valueOf: () => Reducer<S, A>
  concat: <S1, A1>(
    other: ReducerBoxType<S1, A1>,
  ) => ReducerBoxType<S & S1, A | A1>
}

const ReducerBox = <S = EmptyState, A = EmptyAction>(
  f: Reducer<S, A> = identity,
): ReducerBoxType<S, A> => ({
  valueOf: () => f,
...
const reduceIncs: Reducer<{ num: number }, IncAction> = (state, action) =>
  action.type === 'inc' ? over(num, x => x + 1, state) : state

const reduceMult: Reducer<{ num: number }, IncAction> = (state, action) =>
  action.type === 'inc' ? over(num, x => x * 2, state) : state
