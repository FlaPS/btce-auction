...
import { accountInfoMock, AccountInfoVO } from '../../../store/api/explorer/accounts'
import { connect } from 'react-redux'
import { AsyncState } from '@sha/fsa'
import { Spinner } from '../../elements/Spinner'
import { MyAccountInfo } from './MyAccountInfo'
import { CheckAccountInfo } from './CheckAccountInfo'
import { AccountInfoTabBar } from './AccountInfoTabBar'

const scatterSelector = (state: FrontState) => state.app.scatter

const ScatterViewRaw = ({scatter}) => {

  const dispatch = useDispatch()
  const scatterIsAttached = scatter.attached

  let [selectedTab, setSelectedTab] = React.useState(0)

  if (selectedTab === 1 && !scatterIsAttached)
    selectedTab = 0
        ...
        <div className='sidebar__content_block-head'>
          {
            scatterIsAttached
              ? <div className='scatter__block'>
                  <img className='scatter__logo' src='/assets/logo.jpg' alt=''/>
                  <div className='scatter__name-wrap'>
                    <div className='scatter__name'>{scatter.scatterData.account}</div>
              ...
              : <div className='scatter__block'>
                  <img className='scatter__logo' src='/assets/logo.jpg' alt=''/>
                  <div className='scatter__name-wrap'>
                    <div className='scatter__name'></div>
                    <div className='scatter__status-wrap'>
                      <div className='scatter__status'></div>
                      <div
                        className='scatter__attach'

                        onClick={() => dispatch(scatterDuck.actions.attach.started(undefined))}
                      >
                        Attach Scatter
                      </div>
                    </div>
...
export const ScatterView = connect(
  (state: FrontState) => ({scatter: state.app.scatter}),
)(ScatterViewRaw)
